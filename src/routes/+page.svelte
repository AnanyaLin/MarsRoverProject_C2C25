<script>
    ...
    const cameraOptions = [
        ...
        { value: 'MAST', name: 'Mast Camera Zoom - Right' },
    ...
    async function fetchPhotos() {
        ...
        if (!startDate && !endDate) {
            for (const rover of rovers) {
                let url = `https://api.nasa.gov/mars-photos/api/v1/rovers/${rover}/latest_photo
s?api_key=${import.meta.env.VITE_NASA_API_KEY}`;
        ...
        for (const rover of rovers) {
            let url = `https://api.nasa.gov/mars-photos/api/v1/rovers/${rover}/photos?api_k
ey=${import.meta.env.VITE_NASA_API_KEY}`;
            url += `&earth_date=${end}`;
            if (selectedCamera !== 'all') {
                url += `&camera=${selectedCamera}`;
            }
...
<p>
    *If you leave everything blank, you will get the photos from the last 7 days from all rovers and all cameras.
</p>